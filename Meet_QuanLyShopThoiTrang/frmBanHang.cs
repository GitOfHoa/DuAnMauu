    ...
    public partial class frmBanHang : Form
    {
        ...
        private void cbTenSP_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (DataRow R in dt.Rows)
            {
                ...
                decimal temp2 = Convert.ToDecimal(txtDonGia.Text);
                txtDonGia.Text = temp2.ToString("###,###,###");
            }
        ...
        private void btXoa_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {                 
                    dtDSCT.Rows.RemoveAt(dgvCTHD.CurrentRow.Index);
                    dgvCTHD.DataSource = dtDSCT;
                    txtTongHD.Text = Convert.ToString(float.Parse(txtTongHD.Text) + float.Parse(dgvCTHD.Rows[dgvCTHD.CurrentRow.Index].Cells[4].Value.ToString()));
                    txtTongThanhToan.Text = Convert.ToString(float.Parse(txtTongHD.Text) +
                           (float.Parse(txtTongHD.Text) * int.Parse(cbThue.Text) / 100)); 
